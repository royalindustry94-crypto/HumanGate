...
Task 1: complete (commits 585eb52..caec2ec, tests: production asset contract 4/4 + JUnit + lint + debug APK -> PASS in Leon Android APK run 71)\nTask 2: complete (commits caec2ec..9da2706, tests: ProductionAssetContractTest + LeonMeshRigTest + full JUnit + lint + debug APK -> PASS in Leon Android APK run 80)\nTask 3: complete (commits 616e609..8aff666, tests: LeonPuppetRendererContractTest RED in run 83 -> production swap -> full production asset tests + JUnit + lint + debug APK PASS in Leon Android APK run 91)\nTask 1: complete (production asset Python contract 4/4 passed in Leon APK run 69; :app:generateLeonProduction executed before JVM tests. Overall run remained red only because legacy PhotoAssetContractTest still targets the deliberately retired corrupt WebP and is replaced in Task 2.)
Task 4 Ruling: raw sdkmanager/emulator orchestration is unreliable on the hosted runner (first missing sdkmanager from PATH, then emulator boot stalled). Replace those shell-managed lifecycle steps with ReactiveCircus/android-emulator-runner pinned to commit a421e43855164a8197daf9d8d40fe71c6996bb0d, while keeping our own ADB install/screenshot/validator script. Cost if wrong: CI depends on one pinned third-party testing action; Android production code/runtime remains dependency-free.
Task 4 Ruling: emulator shell cannot start LeonOverlayService because the service is correctly non-exported. Keep it non-exported and add a BuildConfig.DEBUG-only MainActivity test extra that starts the service from Leon's own app UID. Cost if wrong: debug-only CI launch path; production service exposure remains unchanged.
