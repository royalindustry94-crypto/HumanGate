...
public final class LeonVoiceRecorder {
    ...
    public boolean start(Listener listener) {
        // Claim the recording slot atomically: two concurrent start() calls must not both pass the
        // check and race to open the mic, or the losing AudioRecord would leak (never released).
        ...
        try {
            // Only buffer one minBuffer chunk, not several: the whole point of MAX_DURATION_MS is a
            // hard cost ceiling, so a single read must not be able to overshoot it by a multi-chunk
            // (potentially seconds-long) block of audio.
