...
|---|---|
| Severity | HIGH |
| Evidence | `vercel.json` routes `/api/*` to the public Vercel API deployment, `apps/api/api/index.py` is explicitly a Vercel serverless entrypoint, and `docs/ops/DEPLOYMENT.md` / `README.md` both now describe the API as stateless on Vercel while only the separate `automation` service stays durable. The old limiter in `apps/api/app/core/rate_limit.py` stored counters in each process's in-memory `OrderedDict`, so concurrent Vercel instances did **not** share request budgets. `apps/api/app/main.py` attached that middleware globally to every route except `/health/*` and `/metrics`, with a stricter `/auth/*` tier; that meant the live gap affected both abuse-sensitive local auth (`/auth/signup`, `/auth/login`) and general API routes including spend/content-job surfaces. |
| Fix | `apps/api/app/core/rate_limit.py` now adds `PostgresRateLimiter`, which keeps the existing fixed-window limits and audit logging contract but stores counters in a shared `request_rate_limits` PostgreSQL table (`apps/api/alembic/versions/0058_request_rate_limits.py`). `apps/api/app/main.py` now uses that shared limiter for the live middleware, so all stateless API instances enforce the same per-IP budget without introducing Redis or any other new infrastructure dependency. |
| Tests | `apps/api/tests/test_rate_limit.py` now proves the budget is shared across two separately constructed middleware/app instances backed by the same database, and that expired `request_rate_limits` rows are cleaned up. Targeted validation on this branch: `alembic upgrade head`, `python -m pytest tests/test_rate_limit.py -q`, and `python -m ruff check app/core/rate_limit.py app/main.py tests/test_rate_limit.py`. |
| Status | **CLOSED.** The risk was live after the API moved to Vercel's stateless runtime, not merely dormant. Counters now live in PostgreSQL, which the stack already operates and which every replica can share safely. |
### TD-091 — Worker concurrency / lease-renewal follow-up — **CLOSED (2026-09-16)**

| Field | Value |
|---|---|
| Severity | MEDIUM |
| Evidence | `apps/api/app/orchestration/claiming.py:182-385` already serializes the actual claim path: the worker registry row is locked with `with_for_update=True`, candidate assignments are selected `FOR UPDATE SKIP LOCKED`, and only `PENDING` rows are mutated to `DISPATCHED`, so two workers do **not** concurrently claim the same pending assignment. The real recovery behavior lives in `apps/api/app/orchestration/recovery.py:293-324`: any `DISPATCHED`/`ACKNOWLEDGED` row whose `lease_expires_at < now` is requeued, regardless of whether the worker process is otherwise still alive. The production worker service in `apps/worker/worker/main.py:38-78` runs real Draft Desk work through `ReferenceWorkerClient`, and the executable stages are the live `scripting` / `idea` jobs (`apps/worker/worker/client.py:22-23`). |
| Real exposure | The investigation found a **false lease-expiry reliability bug, not a double-claim bug**. Before this fix, the reference worker acknowledged the assignment, executed the stage, and only renewed at the tail end of `run_one`, so a healthy slow/hung `scripting` or `idea` execution could outlive its 60s lease and be reclaimed mid-run. Another worker could then re-claim the same assignment attempt path while the first worker was still executing. Existing safeguards already bounded duplicate provider side effects: provider-effect keys are stable per assignment across retries (`apps/api/app/orchestration/provider_effects.py:8-19`) and the reference worker refuses to re-execute when a prior attempt already reserved that key (`apps/worker/worker/client.py:275-301`). So the live impact here was wasted worker compute, consumed retry attempts, and possible false failure/dead-lettering of slow jobs — not an unbounded duplicate-billing hole. |
| Fix | `apps/worker/worker/client.py:79-105` now computes a renewal cadence and renews the assignment in the background for the full executor runtime (`apps/worker/worker/client.py:258-332`), instead of waiting until the end. `apps/worker/worker/core/config.py:38-47` and `apps/worker/worker/main.py:38-48` now thread the worker's configured lease length into the live client so renewal timing stays aligned with the deployment's lease configuration. This is the smallest fix because the claim/recovery DB design already correctly prevented concurrent claims and already recovered crashed workers; only the live worker execution window was under-renewed. |
| Tests | Added `test_reference_worker_client_renews_lease_during_slow_execution` in `apps/api/tests/test_reference_worker_client.py:416-535`, plus `test_reference_worker_client_submits_executor_failures` to keep the worker contract explicit when the executor itself raises. The slow-execution test shrinks the lease to 1s, starts a real in-flight worker execution, keeps that executor blocked past the original lease window, then proves the lease reaper does **not** recover that still-running assignment, a rival worker cannot claim it, and the original worker still completes the run successfully. Targeted validation on this branch: `alembic upgrade head`, `pytest tests/test_reference_worker_client.py -q` (4 passed), `pytest tests/test_stage_claiming_ws2.py tests/test_lease_recovery_ws3.py tests/test_reference_worker_client.py -q` (61 passed), `pytest` in `apps/worker` (10 passed), `ruff check apps/api/tests/test_reference_worker_client.py apps/worker/worker/client.py apps/worker/worker/main.py apps/worker/worker/core/config.py`, and `ruff format --check` on the same files. |
| Status | **CLOSED.** The claim path was already concurrency-safe and crash recovery already existed; the real exposure was bounded to long-running in-flight worker executions losing their lease. The new background renewal loop closes that gap without weakening recovery, spend controls, provider abstraction, or audit behavior. |
### TD-092 — Operations Dashboard N+1 / polling / export pagination — **CLOSED (2026-09-16)**

| Field | Value |
|---|---|
| Severity | MEDIUM |
| Evidence | The dashboard route surface in `apps/api/app/api/routes/operations_dashboard.py` polls the same expensive cards repeatedly (`/workers`, `/worker-timeline`, `/leads`, etc.). In `apps/web/src/LumoraDashboard.tsx:249` the refresh cadence is a fixed `AUTO_REFRESH_MS = 20_000`, and refresh loops at `:1856-1861`, `:1898-1902`, and `:2045-2049` re-fetch visible views every 20s (guarded only by tab visibility + in-flight guard, with no interval backoff). The worker-monitor N+1 called out in commit `e92be1d` is already fixed, but `apps/api/app/services/operations_mission.py` still had a second per-worker query loop in `worker_timeline` (`select(WorkerRegistration)` once, then inside `for worker in workers`, another `select(StageAssignment)...limit(20)` per worker at `:610-625`). Separately, `apps/api/app/services/operations_dashboard.py` listed leads with no server-side bound (`list_leads`, old `stmt.order_by(...); session.execute(stmt)` path), so one request could materialize all matching leads. |
| Real exposure | At W workers, the old timeline path executed `1 + W` queries per request. With W=100 workers and the existing 20-second UI refresh, one open dashboard tab would drive ~101 queries every poll cycle (~5.05 queries/second) for just this card. The old leads list had no limit, so a workspace with 10,000 leads would serialize all 10,000 rows every refresh of the Leads view; with a 20-second poll, that repeats unbounded JSON materialization/network transfer even when only the first screen is visible. |
| Fix | `apps/api/app/services/operations_mission.py` now replaces the `worker_timeline` per-worker assignment loop with `_recent_assignments_by_worker(...)`, a set-based window query (`row_number() over (partition by worker_id order by updated_at desc, id desc)`) that pulls at most 20 recent assignments per worker in one round trip. `apps/api/app/services/operations_dashboard.py` + `apps/api/app/api/routes/operations_dashboard.py` now bound `/operations/leads` with explicit pagination (`limit` default 200, max 500; `offset` default 0, max 10,000), apply `ORDER BY updated_at DESC, id DESC`, and compute `total` via a separate count so clients can page without unbounded materialization. |
| Tests | `apps/api/tests/test_dashboard_query_efficiency.py` adds `test_worker_timeline_query_count_does_not_grow_with_worker_count`, asserting the timeline query count is worker-cardinality invariant (and ≤2 statements) to prevent N+1 regressions. `apps/api/tests/test_operations_dashboard_v2.py` adds `test_leads_list_is_paginated_and_reports_full_total`, seeding 260 leads and proving default cap behavior (200 rows returned, `total=260`) plus non-overlapping paged retrieval (`limit=50&offset=200`). Targeted validation on this branch: `python -m pytest apps/api/tests/test_dashboard_query_efficiency.py apps/api/tests/test_operations_dashboard_v2.py -q` and `python -m ruff check apps/api/app/api/routes/operations_dashboard.py apps/api/app/services/operations_dashboard.py apps/api/app/services/operations_mission.py apps/api/tests/test_dashboard_query_efficiency.py apps/api/tests/test_operations_dashboard_v2.py`. |
| Status | **CLOSED.** The remaining dashboard N+1 and unbounded lead materialization paths are now bounded, and the fixed 20-second polling interval is no longer paired with per-worker or unbounded-row query growth on these endpoints. |
...
### TD-094 — Browser security headers / session design — **CLOSED (2026-09-15)**

| Field | Value |
|---|---|
| Severity | MEDIUM |
| Evidence | Before this pass, neither API nor web deployment config set browser security headers: `apps/api/app/main.py` configured CORS, request IDs, and rate limiting but had no CSP/HSTS/nosniff/frame/referrer headers; `vercel.json` only defined build settings and `/api/*` rewrite with no `headers` section. Session design is bearer-token based rather than cookie-session based: local auth returns `access_token`/`token_type`/`expires_in` JSON (`apps/api/app/api/routes/auth.py`, `apps/api/app/services/local_auth.py`), the web client forwards that token in an `Authorization` header (`apps/web/src/api.ts`), and stores it in per-tab `sessionStorage` (`apps/web/src/App.tsx`) with an explicit comment that persistent refresh/revocation-capable sessions are not present. |
| Real exposure | Missing anti-framing headers meant review/login surfaces could be embedded by a malicious origin (clickjacking risk). Missing CSP on the web shell meant any script injection bug in that origin would run unrestricted and could exfiltrate the in-tab bearer token from `sessionStorage`, then call workspace APIs as that user until expiry. Missing HSTS/nosniff/referrer policy removed transport and browser-hardening constraints that reduce downgrade, MIME confusion, and cross-origin metadata leakage risk. |
| Fix | Added API-side response hardening in `apps/api/app/main.py`: a centralized `_attach_security_headers(...)` now sets `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, and `Strict-Transport-Security` on all API responses, plus an API-safe CSP (`default-src 'none'; frame-ancestors 'none'; base-uri 'none'; form-action 'none'`) on non-doc routes, including 500 responses via `_unhandled_exception_handler`. Added web-deployment header policy in `vercel.json` with CSP (`frame-ancestors 'none'`, restricted sources including Google Fonts), HSTS, nosniff, frame deny, and referrer policy so browser-delivered frontend pages ship explicit protections. |
| Tests | Added `apps/api/tests/test_security_headers.py`: `test_api_responses_include_browser_security_headers` and `test_not_found_responses_still_include_browser_security_headers` assert concrete API header values on both success and framework-generated 404 errors, `test_auth_mode_response_does_not_set_session_cookie` pins the no-cookie session contract, and `test_vercel_web_responses_define_browser_security_headers` fails if frontend security headers are removed from `vercel.json`. Targeted validation on this branch: `python -m pytest tests/test_security_headers.py -q` and `python -m ruff check app/main.py tests/test_security_headers.py`. |
...
| Fix | Added explicit least-privilege workflow permissions for CI (`permissions: contents: read`) so all CI jobs run with a read-only token by default regardless of repository-level defaults (`.github/workflows/ci.yml:9-10`). Added a regression meta-check in the existing security guard script to fail CI if any workflow introduces a third-party `uses:` ref without an immutable pin (full 40-character commit SHA for repository actions, or `@sha256:` digest for `docker://` actions), to inspect nested `uses:` entries in local composite actions under `.github/actions/**/action.yml`, and to fail if CI permissions drift to write access (`.github/workflows/ci.yml:223-369`). |
| Tests | Regression coverage is enforced in the existing required `security` CI job via the updated "Validate Claude/Codex exact-head gate" script (`.github/workflows/ci.yml:223-369`). Local validation in this pass executed the same new invariant logic directly with a Python probe from repo root (checks passed: CI read-only permissions policy and immutable pin enforcement across workflows and composite-action manifests). |
...
| Fix | `app/core/rate_limit.py`: per-IP fixed-window limiter with the same live contract as the original TD-034 fix — global limit (default 300 req/60s per IP) on every route except `/health/*` and `/metrics`, stricter dedicated limit (default 10 req/60s per IP) on `/auth/signup` and `/auth/login`, `429` + `Retry-After` on rejection, audit log event `rate_limit_exceeded`, and keys based on `request.client.host` rather than spoofable `X-Forwarded-For`. The original in-process `InMemoryRateLimiter` remains for direct tests/single-process use, but TD-090 moved the live middleware to `PostgresRateLimiter` backed by shared `request_rate_limits` rows so stateless Vercel replicas enforce one shared budget without adding Redis. Settings remain `RATE_LIMIT_ENABLED`, `RATE_LIMIT_WINDOW_SECONDS`, `RATE_LIMIT_REQUESTS_PER_WINDOW`, and `AUTH_RATE_LIMIT_REQUESTS_PER_WINDOW`. |
