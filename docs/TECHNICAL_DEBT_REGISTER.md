...
| Tests | Added `apps/api/tests/test_security_headers.py`: `test_api_responses_include_browser_security_headers` asserts concrete API header values, `test_auth_mode_response_does_not_set_session_cookie` pins the no-cookie contract, `test_vercel_web_responses_define_browser_security_headers` fails if frontend security headers are removed from `vercel.json`, and `test_frontend_uses_bearer_header_and_tab_scoped_session_storage` guards the bearer + tab-scoped session design. Targeted validation on this branch: `python -m pytest tests/test_security_headers.py -q` and `python -m ruff check app/main.py tests/test_security_headers.py`. |
